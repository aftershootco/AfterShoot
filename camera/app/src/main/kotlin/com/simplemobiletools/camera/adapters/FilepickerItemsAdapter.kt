package com.simplemobiletools.camera.adapters
...
import com.simplemobiletools.camera.activities.BaseSimpleActivity
import com.simplemobiletools.camera.adapters.MyRecyclerViewAdapter
            ...
            if (fileDirItem.isDirectory) {
                ...
                if (!activity.isDestroyed && !activity.isFinishing) {
                    if (itemToLoad.toString().isGif()) {
                        ...
                        Glide.with(activity).load(itemToLoad).transition(DrawableTransitionOptions.withCrossFade()).apply(options).into(list_item_icon)
                    }
                }
            }
        }
